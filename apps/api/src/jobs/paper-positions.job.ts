 ...
 */
export async function handlePaperPositionUpdate(job: Job): Promise<{ updated: number }> {
 ...
 */
export async function handlePositionReconciliation(job: Job): Promise<{ synced: number; drifts: number; errors: number }> {
  ...
  return { synced: result.synced, drifts: result.drifts.length, errors: result.errors.length };
}
